...
class mosseCACF_sparse_Ver1:
    ...
    def start_tracking(self):
        ...
        patch_info = self._get_subwindow(init_frame, pos, window_sz, 0)
        patch = patch_info[0]
        ...
        patch_info_for_DL_training = self._get_subwindow(init_img_training, pos, target_sz, 0)
        ...
        patch_for_DL_training_sparse = patch_for_DL_training
        ...
        #datagen = ImageDataGenerator(horizontal_flip=True, width_shift_range=0.2, zoom_range = 0.15, height_shift_range = 0.2)
        ...
        patch_info_for_DL_test_forgnd = self._get_subwindow(init_img_training, pos, window_sz_DL_testing, 0)
        ...
        for val in range(len(offset)):
            patch_info_for_DL_background = self._get_subwindow(init_img_training, np.add(pos,offset[val]), target_sz, 0)
        ...
        for val in range(len(offset_background)):        
            patch_info_for_DL_background_bigwindow = self._get_subwindow(init_img_training, np.add(pos,offset[val]), target_sz,0)
            ...
            datagen = ImageDataGenerator(horizontal_flip=0.3, width_shift_range=[-6,6])
        ...
        #Generate foreground data to save in train folder.
        #To generate foreground test images, select main patch, randomly select any one channel (r,g, or b), create a random sparse matrix with values zeros and ones
        # multiply the selected channel and then concatenate with the actual image. Indicates one channel of that image will be sparse. 
        sparse_values = np.arange(0.2, 0.7, 0.1)
        row_img_converted_to_gray= patch_for_DL_training_sparse.shape[0]
        col_img_converted_to_gray=patch_for_DL_training_sparse.shape[1]
        sparse_extracted=0
        sparse_patch = patch_for_DL_training_sparse
        for sparse_img_values in sparse_values:
            s = sparse.random(row_img_converted_to_gray, col_img_converted_to_gray, density=sparse_img_values)
            sparse_array = s.A
            sparse_array[sparse_array>0.1]=1
            sparse_layer = random.randint(0,2)
            new_patt = patch_for_DL_training[:,:,sparse_layer]
            sparse_img = np.multiply(new_patt,sparse_array)
            ##sparse_patch[:,:,sparse_layer] = sparse_img
            if sparse_layer == 0:
                new_sparse = np.dstack((sparse_img,sparse_patch[:,:,1],sparse_patch[:,:,2]))
            elif sparse_layer == 1:
                new_sparse = np.dstack((sparse_patch[:,:,0], sparse_img, sparse_patch[:,:,2]))
            else:
                new_sparse = np.dstack((sparse_patch[:,:,0],sparse_patch[:,:,1],sparse_img))

            cv2.imwrite(os.path.join(train_foreground , 'foreground_sparse'+str(sparse_extracted)+'.jpg'), new_sparse)
            sparse_extracted= sparse_extracted+1
        ...
        for idx in range(len(self.frame_lists)):
            if idx == 0:
                ...
                patch_info = self._get_subwindow(frame_gray, pos, window_sz, 0)
                patch = patch_info[0]
                ...
                patch_info_from_correlation_for_evaluation = self._get_subwindow(current_frame, pos, target_sz, 0)
                ...
                #ssim_value_correlation = measure.compare_ssim(np.array(original_patch_for_comparison), np.array(patch_for_ssim_correlation_output), multichannel=True)
                ...
                if pred1[0][1] < 0.8 or ssim_value_correlation < 0.18:
                ...
                if shall_i_continue_prediction ==1:
                    if (pred1[0][1]< 0.8) or (ssim_value_correlation < 0.18):
                        for scale_variations_img in range (0,len(targert_sz_with_scale_variations)):
                            ...
                            patch_info_scaled = self._get_subwindow(current_frame, pos, target_sz_scaled, 1)
                            ...
                            if pred_scaled[0][1] > 0.5:
                                ...
                                ssim_value_scaled = measure.compare_ssim(np.array(previous_patch_old), np.array(patch_for_ssim_scaled), multichannel=True)
                                if ssim_value_scaled >= 0.18:
                                    shall_i_continue_prediction = 0
                                    results_prediction.append(pred_scaled)
                                    #width_for_accuracy_calculation = target_sz_scaled[1]
                                    #height_for_accuracy_calculation = target_sz_scaled[0]
                                    break
                                else:
                                    shall_i_continue_prediction = 1
                ...
                if shall_i_continue_prediction == 1:
                    ...
                    loc1 = images.shape[0]
                    loc2 = images.shape[1]
                    overlap_row = int(np.ceil(np.divide(target_sz[1],3)))
                    overlap_col = int(np.ceil(np.divide(target_sz[0],3)))
                    ...
                    if starting_point_x < 0:
                        starting_point_x =0
                        ending_point_x = loc1
                    if ending_point_x > loc2:
                        starting_point_x = 0
                        ending_point_x = loc2
                    if starting_point_y < 0:
                        starting_point_y =0
                        ending_point_y = loc1
                    if ending_point_y > loc1:
                        starting_point_y =0
                        ending_point_y = loc1
                    ...
                    if starting_point_y ==0 and ending_point_y ==0:
                        ...
                        ending_point_y = loc1
                    if starting_point_x == loc2 and ending_point_x == loc2 :
                        #starting_point_x =np.abs(loc2 - target_sz[1])
                        starting_point_x =0
                        ending_point_x = loc2
                    if starting_point_y ==loc1 and ending_point_y ==loc1:
                        starting_point_y =np.abs(loc1 - target_sz[0])
                        starting_point_y =0
                        ending_point_y = loc1
                    ind_DL_context = 1 
                    for loc_val in range(starting_point_x,ending_point_x,overlap_row):
                        for loc_col_val in range(starting_point_y,ending_point_y,overlap_col):
                            ...
                            Multiple_patched_info_from_context_DL = self._get_subwindow(images, location, target_sz, 1)
                    ...
                    if (position_max[0] ==0) and (shall_i_continue_prediction == 0):
                        ...
                        for max_pos_locations in position_max:
                            ...
                            centre_location_y_new_frame = int(np.floor(np.add(final_loc_col_,np.divide(target_sz[0],2))))
                            centre_location_x_new_frame = int(np.floor(np.add(final_loc_row_,np.divide(target_sz[1],2))))
                            patch_for_ssim = self._get_subwindow(current_frame, [centre_location_y_new_frame, centre_location_x_new_frame], target_sz, 1)
                        ...
                        if len(boundingBoxes) < 1:
                            ...
                            for selected_paches in range(0, len(boundingBoxes)):
                                ...
                                if checking_for_false_positive_in_bbboxes ==0:
                                    x_end_value = int(np.add(new_patch_from_bbbox[0], target_sz[1]))
                                    y_end_value = int(np.add(new_patch_from_bbbox[1], target_sz[0]))
                        ...
                        for selected_paches in range(0, len(boundingBoxes_new)):
                            ...
                            patch_for_ssim = self._get_subwindow(current_frame, pos, target_sz, 1)
                            patch_for_ssim_modified = patch_for_ssim[0].astype(np.uint8)
                            patch_for_ssim_resized = cv2.resize(patch_for_ssim_modified, (int(target_sz[1]),int(target_sz[0])))
                        ...
                        if len(max_value_in_ssim_values) != 0:
    ...
    def iou_computation_CACFmethod(self, val):  
        ...
        for i in range(0, len(ground_values)): 
            ...
            c2 =[]
            ci = []
            #r1 = [(gt_row[0]), (gt_row[1]), gt_row[2], gt_row[3]]
            r1 = [(gt_row[0]), (gt_row[1]), mosse_out[2], mosse_out[3]]
            #mosse_out[2] = np.add(mosse_out[2],10)
            #mosse_out[3] = np.add(mosse_out[3],10)
            #r1 = [(gt_row[0]+np.divide(gt_row[2],2)), (gt_row[1]+np.divide(gt_row[3],2)), gt_row[2], gt_row[3] ]
            #r1 = [(gt_row[1]+np.divide(gt_row[3],2)), (gt_row[0]+np.divide(gt_row[2],2)), mosse_out[2], mosse_out[3] ]
            r2 = [(mosse_out[0]), (mosse_out[1]), mosse_out[2], mosse_out[3] ]
            ...
            intermedi1 = np.multiply(r1[2],r1[3])
            intermedi2 = np.multiply(r2[2],r2[3])
            intermedi3 = np.add(intermedi1,intermedi2)
            au = np.subtract(intermedi3,ai)
            #au = np.ceil(np.subtract(np.add(np.multiply(r1[2],r1[3]),np.multiply(r2[2],r2[3]))), ai)
    ...
    def _get_subwindow(self, init_frame, pos, window_sz, corr_or_DL):
        #the fourth parameter corr_or_DL - 0 indicates correlation and 1 indicates DL
        if pos[0]<0:
            pos[0]=0
        if pos[1]<0:
            pos[1]=0
        ...
        patch_endpoint_x = np.add(pos[1],window_sz[1])
        patch_endpopint_y = np.add(pos[0],window_sz[0])
        ########newly added to solve selection of wrong patches
        if pos[0] > height_of_image :
            pos[0] = np.subtract(height_of_image, np.multiply(2,window_sz[0]))
        if pos[1] > width_of_image :
            pos[1] = np.subtract(width_of_image, np.multiply(2, window_sz[1]))
        if corr_or_DL ==1:
            if patch_endpoint_x > width_of_image :
                pos[1] = np.subtract(width_of_image, window_sz[1])
            if patch_endpopint_y > height_of_image :
                pos[0] = np.subtract(height_of_image, window_sz[0])
        ##########################
        ...
        #############newly added###########
        ########check how many values goes less than zero#########
        ...
        if patch_starting_point_y < 0:
            patch_starting_point_y =0
            patch_end_point_y = ys[len(ys)-1]+ len(padding_top)
        if patch_starting_point_x < 0:
            patch_starting_point_x = 0
            patch_end_point_x = xs[len(xs)-1] + len(padding_right)
        if patch_end_point_y > height_of_image:
            patch_end_point_y = height_of_image -1
            patch_starting_point_y = patch_end_point_y - window_sz[0]
        if patch_end_point_x > width_of_image :
            patch_end_point_x = width_of_image -1
            patch_starting_point_x = width_of_image -window_sz[1]
    ...
    def _get_subwindow_old_old_version(self, init_frame, pos, window_sz):
        if pos[0]<0:
            pos[0]=0
        if pos[1]<0:
            pos[1]=0
        ...
        ########newly added to solve selection of wrong patches
        if pos[0] > height_of_image :
            pos[0] = height_of_image - window_sz[0]
        if pos[1] > width_of_image :
            pos[1] = width_of_image - window_sz[1]
        ##########################
        xs = np.floor(pos[1]) + np.arange(0,window_sz[1],1) - np.floor(np.divide(window_sz[1],2))  
        ys = np.floor(pos[0]) + np.arange(0,window_sz[0],1) - np.floor(np.divide(window_sz[0],2))
        #############newly added###########
        ...
        #check any of the padding left/top/bottom/right is greater than zero, indicates some of the values in patch are out of bound.
        if len(padding_left)>0:
            patch_starting_point_x = 0
            patch_end_point_x = patch_starting_point_x + window_sz[0]
    ...
    #lasitha - computing stacked features for all offset locations
        for val in range(len(offset)):
            patch_info = self._get_subwindow(init_frame, np.add(pos,offset[val]), window_sz , 0)
            patch =patch_info[0]
