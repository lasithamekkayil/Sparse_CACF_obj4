...
class mosselas5_CACF:
    ...
    def start_tracking(self):
        ...
        datagen = ImageDataGenerator(rotation_range=20, horizontal_flip=True, width_shift_range=0.2, zoom_range = 0.15, height_shift_range = 0.2,shear_range =0.15, fill_mode = "nearest")
        ...
        for val in range(len(offset)):
            ...
            datagen = ImageDataGenerator(rotation_range=2, horizontal_flip=0.3, width_shift_range=[-6,6])
        ...
        for x in small_list:
            shutil.copy('F:/git_folder/resnet_modified_to_vgg_ver1/train_folder/foreground/'+str(x),'F:/git_folder/resnet_modified_to_vgg_ver1/test_folder/foreground/')
        ...
        for x in small_list_background:
            shutil.copy('F:/git_folder/resnet_modified_to_vgg_ver1/train_folder/background/'+str(x),'F:/git_folder/resnet_modified_to_vgg_ver1/test_folder/background/')
        ...
        for idx in range(len(self.frame_lists)):
            if idx == 0:
                ...
                img_array_ = np.array(patch_from_correlation_for_evaluation)
                ...
                if pred1[0][1]< 0.4:
                    for scale_variations_img in range (0,len(targert_sz_with_scale_variations)):
                        ...
                        if pred_scaled[0][1] > 0.5:
                            ...
                            #file_to_save_location_from_DL.write("\n" + str(col_scaled_starting_location) + " " + str(row_scaled_starting_location))
                            shall_i_continue_prediction = 0 #indicates vgg predicted it properly, so no need to check for location shift
                            break
                ...
                if shall_i_continue_prediction == 1:
                    ...
                    if position_max[0] ==0 :
                        ...
                        if len(selected_patch)==1:
                            pos[1] = np.floor(int(np.add(selected_patch[0][0],np.divide(target_sz[1],2))))
                            pos[0] = np.floor(int(np.add(selected_patch[0][1],np.divide(target_sz[0],2))))
                        else:
